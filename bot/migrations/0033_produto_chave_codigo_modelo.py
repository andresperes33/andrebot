...
def _eh_codigo_modelo(w):
    ...
    if not (re.search(r'\d', w) and re.search(r'[a-z]', w)):
        return False
    if re.fullmatch(r'\d{1,4}k', w):
        return False
    if re.fullmatch(r'\d{1,4}p', w):
        return False
    if re.fullmatch(r'\d{2,4}hz', w):
        return False
    if re.fullmatch(r'\d{2,4}w', w):
        return False
    if re.fullmatch(r'\d{1,4}gb', w) or re.fullmatch(r'\d{1,4}tb', w):
        return False
...
def backfill(apps, schema_editor):
    from bot.services import _linha_titulo, _chave_produto as _cp
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('bot', '0032_reclassificar_jogos_fones'),
    ]
